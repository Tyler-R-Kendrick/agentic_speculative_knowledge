...
class ManifoldSidecar:
    def __init__(self, model_id: str = "heuristic-sidecar-v1", geometry_version: str = "euclidean-baseline-v1"):
        self.model_id = model_id
        self.geometry_version = geometry_version

    def rank_inference_candidates(self, request: RankingRequest) -> RankingResponse:
    ...
    def rank_facet_candidates(self, request: RankingRequest) -> RankingResponse:
        self._require_speculative_branch(request.branch_name)
        run_id = self._run_id()
        ranked = []
        for candidate in request.candidates:
    ...
    def _require_speculative_branch(self, branch_name: str) -> None:
        if not (branch_name.startswith("inference/") or branch_name.startswith("verification/")):
            raise BranchContextError("ranking requires inference/* or verification/* branch context")
